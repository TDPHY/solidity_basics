...
/**
 * @title UUPSAuctionFactory UUPS拍卖工厂合约
 * @dev 使用UUPS（通用可升级代理标准）模式实现的可升级拍卖工厂合约
 *      UUPS模式相比透明代理更节省gas，将升级逻辑放在实现合约中而不是代理合约中
 */
contract UUPSAuctionFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    // 存储所有创建的拍卖
